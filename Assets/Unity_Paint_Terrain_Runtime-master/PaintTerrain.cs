...
public class PaintTerrain : MonoBehaviour
{
    ...
    void CreateMultipleHills(int amount)
    {
        ...
        for (int i = 0; i < amount; i++)
        {
            ...
            int x = Mathf.RoundToInt(Random.Range(0, terrainData.heightmapResolution));
            int z = Mathf.RoundToInt(Random.Range(0, terrainData.heightmapResolution));

            if (x - radius < 0) x = radius;
            if (x + radius > terrainData.heightmapResolution) x = Mathf.RoundToInt(terrainData.heightmapResolution - radius);

            if (z - radius < 0) z = radius;
            if (z + radius > terrainData.heightmapResolution) z = Mathf.RoundToInt(terrainData.heightmapResolution - radius);
    ...
    void FlattenTerrain()
    {
        int xCor = terrainData.heightmapResolution;
        int yCor = terrainData.heightmapResolution;
